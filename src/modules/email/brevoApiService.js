...
const BREVO_SEND_EMAIL_ENDPOINT = `${BREVO_API_URL}/smtp/email`;
...
export const sendOtpEmail = async (email, otp) => {
  ...
  if (!appConfig.email.password) {
    throw new Error('SMTP_PASSWORD (Brevo API Key) is not configured');
  }
  ...
  try {
    ...
    const payload = {
      sender: {
        ...
        email: appConfig.email.fromEmail,
      },
      to: [
        {
          email: email,
      ...
      textContent: `Your verification code is: ${otp}. Do not share this code with anyone. This code will expire in ${appConfig.otp.expiryMinutes} minutes.`,
    };

    const response = await fetch(BREVO_SEND_EMAIL_ENDPOINT, {
      method: 'POST',
      headers: {
        'api-key': appConfig.email.password, // Use API key in header
    ...
    logger.info('OTP email sent successfully via Brevo API', {
    ...
    logger.error('Failed to send OTP email via Brevo API', {
      recipient: email,
      error: error.message,
    });
...
/**
 * Test email connection to Brevo API
...
export const testEmailConnection = async () => {
  ...
  try {
    const response = await fetch(`${BREVO_API_URL}/account`, {
      method: 'GET',
      headers: {
        'api-key': appConfig.email.password,
        'Content-Type': 'application/json',
      },
    });
    ...
    if (!response.ok) {
      const errorData = await response.json();
      logger.error('Brevo API connection test failed', {
        status: response.status,
        error: errorData,
      });
      throw new Error(`Brevo API error: ${response.status} - ${errorData.message || 'Invalid API Key'}`);
    }

    logger.info('Brevo API connection verified successfully');
    return true;
  } catch (error) {
    logger.error('Brevo API connection test failed', {
      error: error.message,
